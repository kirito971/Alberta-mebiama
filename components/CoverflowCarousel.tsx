...
export function CoverflowCarousel({
  ...
  falloff = 0.56,
  fade = 0.1,
  cardWidth = "clamp(160px, 26vw, 280px)",
  gap = 0.08,
  loop = true,
  showCaption = true,
  showPagination = true,
  ...
  const settle = React.useCallback(
    (target: number) => {
      ...
      const step = () => {
        ...
        // ease-out toward the target, not a spring — swap in a spring only
  ...
  return (
    <div
      className={cx("coverflow", className)}
    ...
    >
          ...
          <div className="coverflow-track">
            {slides.map((slide, index) => (
              <div
                ...
                aria-label={`${index + 1} sur ${count} — ${slide.title ?? ""}`}
      ...
      {showCaption && active && (
        <div key={selected} className="coverflow-caption">
          {active.title && <p className="coverflow-title">{active.title}</p>}
          {active.subtitle && (
            <p className="coverflow-subtitle">{active.subtitle}</p>
          )}
          {active.description && (
            <p className="coverflow-description">{active.description}</p>
          )}
          {active.audio && (
            <audio
              key={active.audio}
              ...
              className="coverflow-audio"
              src={active.audio}
            >
              Ton navigateur ne prend pas en charge la lecture audio. Fichier :{" "}
              {active.audio}
            </audio>
          )}
        </div>
        ...
        <div className="coverflow-pagination">
          {slides.map((slide, index) => (
            <button
              key={index}
              type="button"
              aria-label={`Aller à ${slide.title ?? `la démo ${index + 1}`}`}
